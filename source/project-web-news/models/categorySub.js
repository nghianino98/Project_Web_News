...
module.exports = {
    ...
    findByIdAndUpdate: (id,category_name,category_parent) => {
        return new Promise((resolve, reject) => {
            categorySub.findByIdAndUpdate(id,{$set:{categoryName:category_name, categoryMain:category_parent}}).exec((err, succ) => {
                if (err)
                    reject(err);
                else
                    resolve(succ);
